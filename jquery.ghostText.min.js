(function($){$.fn.ghostText=function(opts,args){var $self=this;var val=this.val();var defaults={ghostText:val,ghostClass:'ghost-text',focus:null,blur:null,change:null,clearOnSubmit:true,parentForm:null};var settings=$self.data('ghostText');var methods={init:function(){if(!opts){opts={};}
...
if(settings.blur){settings.blur(val);}},change:function(){val=$self.val();if(settings.change){settings.change(val);}}};if(typeof(opts)=='string'){if(opts in methods){methods[opts](args);}}
else{methods.init();}
return $self;}})(jQuery);